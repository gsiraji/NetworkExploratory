...
G = find_flows(G, Boundary2, V0, G, dyn_vis);
...
G.Nodes.pCount = zeros(numnodes(G),1);
...
k1 = 1;
if make_vid == 1
    v = VideoWriter('pchainsim01.avi');
    v.Quality = 100;
    v.FrameRate = 10;
    open(v)
end
G2 = G;
G2.Edges.Flows = ones(chain_l,1);
G2.Nodes.Potentials = [200;0;(199:-1:1)'];
Nstep = 5000;tot_time = 5000;dt = tot_time/Nstep;
...
for step= 1:1:Nstep
    GID = G2.Edges.ID;
    G2.Nodes.pCount= zeros(numnodes(G2),1);
    G2.Edges.pCount = zeros(numedges(G2),1);
...
%     pall_xy(step,2*p_idx-1:2*p_idx) =  [particle_set(p_idx).comx; particle_set(p_idx).comy];
    ...
    if pedge_num > 0
        edgeind = find(G2.Edges.ID == pedge_num);
        G2.Edges.pCount(edgeind,:) = G2.Edges.pCount(edgeind,:) + 1;
    else
        if current_node_list(p_idx) > 0
            nodeind = find(G2.Nodes.ID == current_node_list(p_idx));
            G2.Nodes.pCount(nodeind,:) = G2.Nodes.pCount(nodeind,:) + 1;
        end
    end
...
%         nexttile;
    ...
    %         colormap(autumn)
    %         p = plot(G.Nodes.comx, G.Nodes.pCount, 'ko');hold on;
            else
                for i  = 1:1:numnodes(G2)
                    rectangle('Position', [G2.Nodes.comx(i,:) 0 1 G2.Nodes.pCount(i,:)./10], 'FaceColor', 'c'); hold on
                    
                end
                for i  = 1:1:numedges(G2)
                    endnodes = G2.Edges.EndNodes(i,:);
                    xval =(G2.Nodes(endnodes(:,1),1).comx + G2.Nodes(endnodes(:,2),1).comx)./2;
                    rectangle('Position', [xval 0 1 G2.Edges.pCount(i,:)./10], 'FaceColor', 'c'); hold on
...
%             G2.Edges.Flows = FindEdgeFlows(G2.Edges, G2.Nodes);
        if chain_mode == 0
                G2 =  find_flows(G2, Boundary2, V0, G,dyn_vis);
...
function G = find_flows(G, Bndry, V, G1,muu )
